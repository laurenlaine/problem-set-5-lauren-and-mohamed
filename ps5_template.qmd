...
```{python}
 districts=gpd.read_file(r"C:\Users\laine\OneDrive\Documents\GitHub\problem-set-5-lauren-and-mohamed\geo_export_eb53b65d-074a-4822-9a10-a283d465ebec.shp") 
```

```{python}
crim_and_civil['District']=crim_and_civil['Agency'].map(lambda x: x.replace("U.S. Attorney's Office,", ""))
crim_and_civil.head()
```

```{python}
enforcement_data_by_district = df_21
...
district_action_counts = enforcement_data_by_district.groupby('District').size().reset_index(name='action_count')
...
census_states = gpd.read_file("C:\\Users\\mmmoh\\DPPS Python\\PSet\\problem-set-5-lauren-and-mohamed\\cb_2018_us_state_500k.shp")

us_attorney_districts = us_attorney_districts.merge(district_action_counts, left_on='district_n', right_on='District', how='left')

us_attorney_districts = us_attorney_districts.to_crs(epsg=4326)  
us_attorney_districts_geojson = us_attorney_districts.to_json()


alt_chart = alt.Chart(alt.Data(values=us_attorney_districts_geojson)).mark_geoshape().encode(
    color=alt.Color('properties.action_count:Q', title='Enforcement Actions'),
    tooltip=[
        alt.Tooltip('properties.District:N', title='District'),
        alt.Tooltip('properties.action_count:Q', title='Enforcement Actions')
    ]
).properties(
    width=800,
    height=500,
