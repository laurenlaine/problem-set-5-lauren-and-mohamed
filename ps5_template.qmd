...
def scrape(year, month):
    ...
    for i in range(250):
        ...
        # Ensure lengths match by iterating over all tags
    ...
    scraped_data=data=pd.DataFrame({'Title':titles, 'Date':dates, 'Category':categories, 'Link':full_links})
...
```{python}
df_23=scraped[scraped['Date'].dt.year==2023]
```

```{python}
scraped.groupby('Category').size()
```
...
```{python}
actions_by_category=alt.Chart(df_21).mark_line().transform_filter(
 'datum.Category==Criminal and Civil Actions or datum.Category==State Enforecement Agencies' ).encode(
  alt.X('Date'),
  alt.Y('count(Title)')
  alt.Color('Category')
)
